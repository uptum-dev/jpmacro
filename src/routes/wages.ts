...
app.get('/real', zValidator('query', realQuerySchema), async (c) => {
  ...
  return c.json({
    ...
    meta: {
      source: rows[0]?.source ?? 'e-stat',
      retrieved_at: latestRetrievedAt,
      note: 'Two series: 1990-2015 (base 2010, e-stat), 2021-present (base 2020, MHLW). Gap: 2016-2020.',
      from,
      to,
...
app.get('/nominal', zValidator('query', nominalQuerySchema), async (c) => {
  ...
  return c.json({
    ...
    meta: {
      ...
      note: 'Base years differ by period: 2001-2005=2000, 2006-2010=2005, 2011-2014=2010',
      from,
      to,
      industry_code: industry ?? 'ALL',
      count: rows.length,
